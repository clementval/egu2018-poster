...
\defverbatim\clawb{%
\begin{lstlisting}
$ clawfc --target=gpu --directive=openacc -o transformed.f90 original.f90
\end{lstlisting}
}
...
\begin{figure}[ht]
  \begin{tikzpicture}
    \tikzstyle{every node}=[font=\small]
    \begin{axis}[
      title = {A) Haswell parallel target vs. Haswell seq target},
  ...
  \begin{tikzpicture}%
    \tikzstyle{every node}=[font=\small]
    \begin{axis}[
      title = {B) Haswell target (ref.) vs. NVIDIA P100 target},
      title style={at={(0.5,-0.15)},anchor=north,yshift=0.5},
      width = 0.45*\textwidth,
      major x tick style = transparent,
      ybar=2*\pgflinewidth,
